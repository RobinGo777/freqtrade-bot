...
# МОДЕЛІ
# ──────────────────────────────────────────────
# Одна стабільна модель, але з кількох ключів
...
async def call_gemini(api_key: str, prompt: str) -> dict:
    """Викликає gemini-2.0-flash з конкретним API ключем."""
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            key_hint = api_key[:8] + "..."
            log.info(f"🤖 Gemini [{key_hint}]: status={resp.status_code}")
            ...
            if resp.status_code == 200:
                ...
                log.info(f"✅ Gemini [{key_hint}]: success")
                return result

            error_text = resp.text
            log.warning(f"⚠️ Gemini [{key_hint}] HTTP {resp.status_code}: {error_text[:200]}")
...
async def generate_content(rubric: str, history: list, extra: dict = None) -> dict:
    ...
    for i, api_key in enumerate(GEMINI_API_KEYS, 1):
        try:
            log.info(f"🔑 Trying Gemini key {i}/{len(GEMINI_API_KEYS)} for [{rubric}]")
            result = await call_gemini(api_key, prompt)
            log.info(f"✅ Content generated via Gemini key {i} for [{rubric}]")
            return result
        except RuntimeError as e:
            if "CRITICAL" in str(e):
                log.error(f"❌ Critical Gemini error with key {i}: {e} — stopping all attempts")
                break
            log.warning(f"⚠️ Gemini key {i} failed for [{rubric}]: {e} — trying next key")
        except Exception as e:
            log.error(f"❌ Unexpected error Gemini key {i} for [{rubric}]: {e}")

    log.error(f"❌ All {len(GEMINI_API_KEYS)} Gemini keys failed for [{rubric}] — SKIPPING POST")
    raise RuntimeError(f"All Gemini keys failed for [{rubric}]")
