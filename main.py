...
# ──────────────────────────────────────────────
SEASON_PHOTOS = {
    "spring": ["spring flowers nature", "cherry blossom spring", "spring morning garden"],
    "summer": ["summer beach ocean", "summer nature sunny", "summer park sunny day"],
    "autumn": ["autumn leaves forest", "fall foliage nature", "autumn park golden"],
    "winter": ["winter snow landscape", "winter forest snow", "winter morning frost"],
}

MONTH_PHOTOS = {
    1:  ["january winter cozy", "new year winter"],
    2:  ["february winter cozy", "winter morning"],
    3:  ["march spring awakening", "first flowers spring"],
    4:  ["april spring nature", "spring rain flowers"],
    5:  ["may spring flowers", "may nature bloom"],
    6:  ["june summer nature", "summer graduation"],
    7:  ["july summer beach", "summer vacation"],
    8:  ["august summer nature", "late summer golden"],
    9:  ["september autumn school", "back to school autumn"],
    10: ["october autumn leaves", "october fall foliage"],
    11: ["november autumn fog", "november cozy indoor"],
    12: ["december winter christmas", "december cozy winter"],
}
...
QUOTE_PHOTOS = [
    ...
    "green nature peaceful",
    "ocean sunrise morning",
    "nature landscape inspiring",
    ...
    for attempt in range(1, 3):
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                log.info(f"🤖 Gemini {model} attempt {attempt}: status={resp.status_code}")
                ...
                if resp.status_code == 200:
                    ...
                    log.info(f"✅ Gemini {model}: success")
                    return result

                error_text = resp.text
                log.warning(f"⚠️ Gemini {model} HTTP {resp.status_code}: {error_text[:200]}")

                if is_critical_error(error_text):
                    log.error(f"❌ Gemini {model}: CRITICAL error — {error_text[:100]}")
                    raise RuntimeError(f"CRITICAL: {error_text[:100]}")

                if resp.status_code in (429, 500, 503):
                    delay = 2 ** attempt
                    log.warning(f"⚠️ Gemini {model}: rate limit/server error, waiting {delay}s")
            ...
            log.error(f"❌ Gemini {model} JSON parse error: {e}")
            if attempt < 2:
                await asyncio.sleep(2)
        except Exception as e:
            log.error(f"❌ Gemini {model} exception: {e}")
            if attempt < 2:
                await asyncio.sleep(2)
    ...
    for attempt in range(1, 3):
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                log.info(f"🤖 Groq attempt {attempt}: status={resp.status_code}")
                ...
                if resp.status_code == 200:
                    ...
                    text = text.strip().removeprefix("```json").removeprefix("```").removesuffix("```").strip()
                    result = json.loads(text)
                    log.info(f"✅ Groq: success")
                    return result

                error_text = resp.text
                log.warning(f"⚠️ Groq HTTP {resp.status_code}: {error_text[:200]}")

                if is_critical_error(error_text):
                    log.error(f"❌ Groq CRITICAL error: {error_text[:100]}")
                    raise RuntimeError(f"CRITICAL Groq: {error_text[:100]}")
...
async def generate_content(rubric: str, history: list, extra: dict = None) -> dict:
    ...
    log.info(f"🤖 Generating content for [{rubric}] extra={extra}")

    for model in GEMINI_MODELS:
        try:
            result = await call_gemini(model, prompt)
            log.info(f"✅ Content generated via Gemini {model} for [{rubric}]")
            return result
        except RuntimeError as e:
            if "CRITICAL" in str(e):
                log.error(f"❌ Critical error with Gemini {model}: {e} — stopping Gemini attempts")
                break
            log.warning(f"⚠️ Gemini {model} failed for [{rubric}]: {e}")
        except Exception as e:
            log.error(f"❌ Unexpected error Gemini {model} for [{rubric}]: {e}")

    log.warning(f"⚠️ All Gemini models failed for [{rubric}] — trying Groq")
    try:
        result = await call_groq(prompt)
        log.info(f"✅ Content generated via Groq for [{rubric}]")
        return result
    except Exception as e:
        log.error(f"❌ Groq also failed for [{rubric}]: {e}")
        raise RuntimeError(f"All AI models failed for [{rubric}]: {e}")
...
def build_daily_phrase(data: dict, photo_b64: str) -> str:
  ...
  <div class="glass-block">
    <div style="font-size:62px; font-weight:800; color:#ffffff;
                text-shadow: 0 2px 12px rgba(0,0,0,0.5); line-height:1.2;">
      {phrase}
    </div>
  </div>
  <div class="glass-block">
    <div style="font-size:48px; font-weight:700; color:#ffffff;
                text-shadow: 0 2px 8px rgba(0,0,0,0.4); line-height:1.3; margin-bottom:20px;">
      {ex_en}
    </div>
    <div style="font-size:48px; font-weight:400; color:rgba(255,255,255,0.92);
...
def build_situation_phrases(data: dict, photo_b64: str, category: dict) -> str:
    ...
    topbar   = f"{category['emoji']} {category['name']}"
    blocks   = ""
    for p in phrases[:5]:
        en = p.get("en", "")
        ua = p.get("ua", "")
        blocks += f"""
  <div class="glass-block" style="padding: 28px 44px;">
    <div style="font-size:42px; font-weight:700; color:#ffffff;
                text-shadow: 0 2px 8px rgba(0,0,0,0.45); line-height:1.25; margin-bottom:10px;">
      {en}
    </div>
    <div style="font-size:40px; font-weight:400; color:rgba(255,255,255,0.90);
                text-shadow: 0 2px 6px rgba(0,0,0,0.4); line-height:1.25;">
      {ua}
    </div>
  </div>"""
    return html_base(photo_b64, topbar, blocks)
...
def build_quote_motivation(data: dict, photo_b64: str) -> str:
    ...
    author_line = f'<div style="font-size:40px; font-weight:700; color:rgba(255,255,255,0.95); margin-top:18px; text-align:right; text-shadow:0 2px 8px rgba(0,0,0,0.4);">— {author}</div>' if author else ""

    blocks = f"""
  <div class="glass-block">
    <div style="font-size:56px; font-weight:800; color:#ffffff;
                text-shadow: 0 2px 12px rgba(0,0,0,0.5); line-height:1.3;">
      "{quote_en}"
    </div>
    {author_line}
  </div>
  <div class="glass-block">
    <div style="font-size:48px; font-weight:400; color:rgba(255,255,255,0.92);
                text-shadow: 0 2px 8px rgba(0,0,0,0.4); line-height:1.35; text-align:left;">
      "{quote_ua}"
    </div>
  </div>"""
...
if __name__ == "__main__":
    ...
    if result.returncode == 0:
        ...
        log.error(f"❌ Chromium install failed: {result.stderr[:300]}")
