...
async def call_groq(prompt: str, model: str) -> dict:
    """Викликає Groq модель як fallback."""
    if not GROQ_API_KEY:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            log.info(f"🤖 Groq {model}: status={resp.status_code}")
            if resp.status_code == 200:
                data = resp.json()
                text = data["choices"][0]["message"]["content"]
                # deepseek-r1 повертає <think>...</think> перед відповіддю — прибираємо
                if "<think>" in text:
                    text = text.split("</think>")[-1].strip()
                text = text.strip().removeprefix("```json").removeprefix("```").removesuffix("```").strip()
                result = json.loads(text)
                log.info(f"✅ Groq {model}: success")
                return result
            error_text = resp.text
            log.warning(f"⚠️ Groq {model} HTTP {resp.status_code}: {error_text[:200]}")
            if is_critical_error(error_text):
        ...
        log.error(f"❌ Groq {model} JSON parse error: {e}")
        raise RuntimeError(f"JSON error: {e}")
    except Exception as e:
        log.error(f"❌ Groq {model} exception: {e}")
        raise RuntimeError(f"Exception: {e}")
        ...
        for i, api_key in enumerate(GEMINI_API_KEYS, 1):
            try:
                ...
                if "CRITICAL" in str(e):
                    log.error(f"❌ Critical Gemini error {model} key {i}: {e} — stopping all")
                    raise
                log.warning(f"⚠️ {model} key {i} failed for [{rubric}]: {e} — trying next")
            except Exception as e:
                log.error(f"❌ Unexpected error {model} key {i} for [{rubric}]: {e}")

    log.warning(f"⚠️ All Gemini models+keys failed for [{rubric}] — trying Groq fallback")

    # Groq fallback
    for groq_model in GROQ_MODELS:
        try:
            log.info(f"🔄 Trying Groq {groq_model} for [{rubric}]")
            result = await call_groq(prompt, groq_model)
            log.info(f"✅ Content generated via Groq {groq_model} for [{rubric}]")
            return result
        except RuntimeError as e:
            if "CRITICAL" in str(e):
                log.error(f"❌ Critical Groq error {groq_model}: {e} — stopping")
                break
            log.warning(f"⚠️ Groq {groq_model} failed for [{rubric}]: {e} — trying next")
        except Exception as e:
            log.error(f"❌ Unexpected Groq error {groq_model} for [{rubric}]: {e}")

    log.error(f"❌ ALL AI models failed for [{rubric}] — SKIPPING POST")
    raise RuntimeError(f"All AI models failed for [{rubric}]")
